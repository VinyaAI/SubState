...
use schema::{
    EntityDef, FieldDef, FieldMode, IdentityDef, RelationDef, SourceDef, SyncSchema, SOURCE_HTTP,
    SOURCE_KAFKA, SOURCE_POSTGRES,
};
use std::collections::HashMap;

/// Default TTL for GPS-like latest-value fields generated by `init`.
pub const DEFAULT_LATEST_VALUE_TTL_MS: u64 = 5000;
...
pub fn attach_kafka_to_entity(
    ...
    for name in &attach.field_names {
        ...
        entity.fields.push(FieldDraft::new(
            name.clone(),
            attach.source_id.clone(),
            mode,
            if mode == FieldMode::LatestValue {
                attach.ordering.clone()
            } else {
                None
            },
...
/// Fill one-hop relations from single-column Postgres FKs when both ends are
/// selected as entities. Multi-column FKs are skipped.
pub fn attach_postgres_relations(entities: &mut [EntityDraft], tables: &[TableInfo]) {
    let mut table_to_entity: HashMap<String, String> = HashMap::new();
    let mut entity_identity: HashMap<String, Vec<String>> = HashMap::new();
    for entity in entities.iter() {
        entity_identity.insert(entity.name.clone(), entity.identity_fields.clone());
        if let Some(table) = entity
            .sources
            .iter()
            .find(|s| s.source_type == SOURCE_POSTGRES)
            .and_then(|s| s.table.as_ref())
        {
            table_to_entity.insert(table.clone(), entity.name.clone());
        }
    }
    if table_to_entity.len() < 2 {
        return;
    }

    let table_by_name: HashMap<&str, &TableInfo> =
        tables.iter().map(|t| (t.name.as_str(), t)).collect();

    for entity in entities.iter_mut() {
        let Some(table_name) = entity
            .sources
            .iter()
            .find(|s| s.source_type == SOURCE_POSTGRES)
            .and_then(|s| s.table.as_ref())
        ...
        for fk in &table.foreign_keys {
            ...
            let remote_col = &fk.referenced_columns[0];
            let Some(target_entity) = table_to_entity.get(&fk.referenced_table) else {
                continue;
            };
            if target_entity == &entity.name {
                continue;
            }
            if !entity.fields.iter().any(|f| &f.name == local) {
                continue;
            }
            let rel_name = local.clone();
            let remote = {
                let default_id = entity_identity
                    .get(target_entity)
                    .and_then(|ids| {
                        if ids.len() == 1 {
                            ids.first()
                    ...
                    .map(String::as_str);
                if default_id == Some(remote_col.as_str()) {
                    None
                } else {
                    Some(remote_col.clone())
                }
            };
            entity.relations.entry(rel_name).or_insert(RelationDef {
...
pub fn build_schema(entities: Vec<EntityDraft>) -> Result<SyncSchema> {
    ...
    for entity in entities {
        ...
        for field in entity.fields {
            ...
            let ttl_ms = field.ttl_ms.or_else(|| {
                if field.mode == FieldMode::LatestValue && is_latest_value_field(&field.name) {
                    Some(DEFAULT_LATEST_VALUE_TTL_MS)
                } else {
                    None
                }
            });
...
mod tests {
    ...
    fn postgres_entity_and_kafka_attach() {
        ...
        assert_eq!(location.flush_ms, Some(100));
        assert_eq!(location.ttl_ms, Some(DEFAULT_LATEST_VALUE_TTL_MS));
    }
    ...
    #[test]
    fn postgres_fk_becomes_one_hop_relation() {
        use postgres_source::ForeignKeyInfo;
        ...
        let drivers = TableInfo {
            ...
            foreign_keys: vec![ForeignKeyInfo {
                columns: vec!["assigned_job".into()],
                referenced_table: "jobs".into(),
                referenced_columns: vec!["id".into()],
            }],
        };

        let mut entities = vec![
            entity_from_postgres_table(&drivers, "driver").unwrap(),
            entity_from_postgres_table(&jobs, "job").unwrap(),
        ];
        attach_postgres_relations(&mut entities, &[drivers, jobs]);
        let schema = build_schema(entities).unwrap();
        let rel = schema
            .entity("driver")
            .unwrap()
            .relations
            .get("assigned_job")
            .expect("assigned_job relation");
        assert_eq!(rel.entity, "job");
        assert_eq!(rel.local, "assigned_job");
        assert_eq!(rel.remote, None);
    }
